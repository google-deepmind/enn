...
class DummySingleIndexLossFn(single_index.SingleIndexLossFn):
  ...
  def __call__(self, apply: base.ApplyFn, params: hk.Params, batch: base.Batch,
...
class AvgSingleIndexLossTest(absltest.TestCase):
  ...
  def test_avgeraging(self):
    ...
    loss, metrics = loss_fn(
        enn=enn, params=dict(), batch=dict(), key=jax.random.PRNGKey(0))
...
class L2LossWithBootstrapTest(absltest.TestCase):
  ...
  def test_null_bootstrapping(self):
    ...
    loss_fn = single_index.L2LossWithBootstrap()
  ...
  def test_zero_bootstrapping(self):
    """Test computed loss is zero when bootstrap weights are zero."""

    boot_fn = lambda data_index, index: np.zeros_like(data_index)
    loss_fn = single_index.L2LossWithBootstrap(boot_fn=boot_fn)
    loss, unused_metrics = loss_fn(
        apply=lambda p, x, i: x[:, :1],
        params=self._params,
        batch=self._batch,
...
class XentLossWithBootstrapTest(parameterized.TestCase):
  ...
  def test_null_bootstrapping(self, num_classes: int):
    ...
    loss_fn = single_index.XentLossWithBootstrap(num_classes)

    batch_size = 4
    batch = {
        'x':
            np.expand_dims(np.arange(batch_size), 1),
        'label':
            np.random.random_integers(0, num_classes - 1, size=(batch_size, 1)),
        'data_index':
            np.expand_dims(np.arange(batch_size), 1),
    }
    ...
    expected_loss = (
        jax.nn.logsumexp(logits) - np.mean(batch['label'] == 0) * logits[0])
    loss, unused_metrics = loss_fn(
        apply=apply, params=self._params, batch=batch, index=self._index)
    self.assertEqual(
        loss, expected_loss,
  ...
  def test_zero_bootstrapping(self, num_classes: int):
    ...
    boot_fn = lambda data_index, index: np.zeros_like(data_index)
    loss_fn = single_index.XentLossWithBootstrap(num_classes, boot_fn)
    batch_size = 4
    batch = {
        'x':
            np.expand_dims(np.arange(batch_size), 1),
